    ...
    public function enAttente(Request $request)
{
    ...
    public function show(Request $request, $id)
    {
        try {
            ...
            $dossier = Dossier::with([
                ...
                'operations.user'  // ✅ Utilise operations au lieu de comments
            ])->findOrFail($id);
            ...
            return view('admin.dossiers.show', compact('dossier', 'agents', 'stats', 'historique'));
    ...
    public function downloadAccuse($id)
    {
        try {
            ...
            $filename = "accuse_reception_{$dossier->numero_dossier}_" . now()->format('Ymd') . ".pdf";
            
            // Log de l'action
            \Log::info("Génération accusé PDF pour dossier {$dossier->id} par " . auth()->user()->name);
    ...
    public function downloadRecepisse($id)
    {
        try {
            ...
            if ($dossier->statut !== 'approuve') {
                return back()->with('error', 'Le récépissé n\'est disponible que pour les dossiers approuvés.');
            }
            
            // Générer le PDF de récépissé
            $pdf = $this->pdfService->generateRecepisseDefinitif($dossier);
            
            $filename = "recepisse_{$dossier->organisation->nom}_{$dossier->numero_dossier}_" . now()->format('Ymd') . ".pdf";
            
            // Log de l'action
            \Log::info("Génération récépissé PDF pour dossier {$dossier->id} par " . auth()->user()->name);
            
            return $pdf->download($filename);

        } catch (\Exception $e) {
            \Log::error('Erreur génération récépissé PDF: ' . $e->getMessage());
            return back()->with('error', 'Erreur lors de la génération du récépissé: ' . $e->getMessage());
        }
    }
...
/**
    ...
    public function downloadRecepisseProvisoire($id)
    {
        try {
            ...
            // Vérifier que le dossier est dans un état approprié
            // Le récépissé provisoire peut être généré pour les dossiers :
            // - soumis (en attente de traitement)
            // - en_cours (en cours de traitement)
            $statutsAutorises = ['soumis', 'en_cours', 'en_attente'];
            
            if (!in_array($dossier->statut, $statutsAutorises)) {
                return back()->with('error', 
                    'Le récépissé provisoire n\'est disponible que pour les dossiers en cours de traitement. ' .
                    'Statut actuel: ' . ucfirst($dossier->statut)
                );
            }
            ...
            if (!$dossier->organisation) {
                return back()->with('error', 
                    'Impossible de générer le récépissé : organisation manquante.'
                );
            }
            
            // Vérifier qu'il y a au moins un fondateur
            if ($dossier->organisation->fondateurs->isEmpty()) {
                return back()->with('error', 
                    'Impossible de générer le récépissé : aucun fondateur déclaré.'
                );
            }
            ...
            $filename = "recepisse_provisoire_{$dossier->organisation->nom}_{$dossier->numero_dossier}_" . 
                       now()->format('Ymd') . ".pdf";
            ...
            \Log::info("Génération récépissé provisoire PDF", [
                'dossier_id' => $dossier->id,
            ...
            // Enregistrer l'activité dans la base de données si vous avez un système d'audit
            if (class_exists('\Spatie\Activitylog\Facades\Activity')) {
                activity()
                    ...
                    ->log('Récépissé provisoire téléchargé');
            }
            ...
            \Log::warning('Tentative d\'accès à un dossier inexistant', [
                'dossier_id' => $id,
                'user_id' => auth()->user()->id,
                'ip' => request()->ip()
            ]);
            
            return back()->with('error', 'Dossier non trouvé.');
            
        } catch (\Exception $e) {
            \Log::error('Erreur génération récépissé provisoire PDF', [
                'dossier_id' => $id,
            ...
            return back()->with('error', 
                'Erreur lors de la génération du récépissé provisoire. ' .
                'Veuillez vérifier les données du dossier et réessayer.'
            );
        }
    }
    ...
    private function sanitizeFilename($filename)
    {
        ...
        $filename = str_replace([' ', 'é', 'è', 'ê', 'à', 'ù', 'ç', 'î', 'ô'], 
                               ['_', 'e', 'e', 'e', 'a', 'u', 'c', 'i', 'o'], 
                               $filename);
        
        // Enlever les caractères non alphanumériques sauf . _ -
        $filename = preg_replace('/[^a-zA-Z0-9._-]/', '', $filename);
        
        // Éviter les noms de fichier trop longs (limite 255 caractères)
    ...
    private function canGenerateRecepisseProvisoire($dossier)
    {
        ...
        return $dossier->organisation && 
               !$dossier->organisation->fondateurs->isEmpty() &&
               in_array($dossier->statut, ['soumis', 'en_cours', 'en_attente']);
    }
    ...
    private function getAvailableActionsUpdated($dossier)
    {
        ...
        // Accusé de réception : toujours disponible
        $actions['accuse_reception'] = true;
        
        // Récépissé provisoire : dossiers en cours de traitement
        $actions['recepisse_provisoire'] = $this->canGenerateRecepisseProvisoire($dossier);
        
        // Récépissé définitif : dossiers approuvés uniquement
        $actions['recepisse_definitif'] = ($dossier->statut === 'approuve');
