...
public function processChunk(Request $request)
{
    try {
        ...
        $processed = 0; // Nombre d'éléments traités
        $imported = 0;  // Nombre d'éléments importés avec succès
        ...
        return response()->json([
            'success' => true,
            'message' => "Chunk traité avec succès",
            
            // ✅ FORMAT ATTENDU par l'interface chunking
            'data' => [
                'processed' => $processed,
                'imported' => $imported,
                'errors' => count($errors),
